...
export = (bot: Client) => {
	const load = (directories: string) => {
		...
		try {
			...
			return console.error(`${chalk.redBright('ERROR!')} The event folder "${directories}" couldn't be loaded.\n${chalk.redBright('ERROR!')} Please ensure a file is added in it to be loaded.`);
		}
		...
		for (const eventFile of events) {
			...
			try {
				...
				return console.error(`${chalk.redBright('ERROR!')} The event file "${eventFile}" couldn't be loaded.\n${chalk.redBright('ERROR!')} Please ensure an export is included within the file.`);
			}
		}
	};
